...
export default function CashierPanelScreen({ navigation }: any) {
  ...
  const fetchOrders = async () => {
    try {
      const { data, error } = await supabase
        .from('orders').select('id, table_no, status, total_amount').eq('restaurant_id', restaurantId)
  ...
  const handleSplitBillPay = async () => {
    ...
    try {
      ...
      const { data: updatedItems, error: fetchError } = await supabase
        .from('order_items').select('id, is_paid')
    ...
    try {
      ...
      if (!targetOrderId) {
        ...
        const { data: newOrder, error: orderError } = await supabase
          .from('orders').insert({ restaurant_id: restaurantId, 
